...
/// Eviction policy that evicts the most recently used cache line.
export default class MostRecentlyUsedEvictionPolicy implements IEvictionPolicy
{
	...
	public getCacheLineToEvict(cacheLineIndices: number[]): number
	{
		...
		// Use a value below the minimum possible access time value as the
		//   initial value
		let highestLastAccessTime = -1
		...
		for (let i = 0; i < cacheLineIndices.length; i++)
		{
			...
			// Keep track of the cache line with the highest access time
			if (lastAccessTime > highestLastAccessTime)
			{
				cacheLineToEvict = cacheLineIndex;
				highestLastAccessTime = lastAccessTime;
			}
		}
